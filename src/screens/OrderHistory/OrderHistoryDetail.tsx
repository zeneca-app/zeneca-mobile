...
const OrderDetailsScreen: React.FC<OrderHistoryDetailScreenProps> = ({
  route,
}) => {
        ...
        <View className="space-y-1">
          ...
          <View className="flex-row justify-between items-center py-3 border-b border-[#1C1C1E]">
            <Text className="text-gray-400 text-base">
              {t("orderHistoryDetail.status")}
            </Text>
            <View className="flex-row items-center gap-2">
              <View
                className={`w-2 h-2 rounded-full ${getStatusColor(orderDetails.status as OrderStatus)}`}
              />
              <Text
                className={`${getStatusTextColor(orderDetails.status as OrderStatus)} text-base`}
              >
                {orderDetails.status}
              </Text>
            </View>
