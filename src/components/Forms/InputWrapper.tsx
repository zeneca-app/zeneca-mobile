...
const InputWrapper = ({
  ...
  return (
    <View className="flex items-stretch">
      ...
      <View
        className={`h-px rounded-full ${isErrored ? "bg-semantic-danger" : "bg-[#313036]"}`}
      />
      <View className="h-6 w-full">
        {Boolean(error) ? (
          <Text className="text-semantic-danger caption-l">{error}</Text>
        ) : (
          <Text className="text-gray-70 caption-l">{hint}</Text>
        )}
      </View>
    </View>
  );
